...
AB_TYPE A_vals[DIM_SEL][DIM_SEL];
AB_TYPE B_vals[DIM_SEL][DIM_SEL];
C_TYPE output[DIM_SEL][DIM_SEL];
C_TYPE output_reference[DIM_SEL][DIM_SEL];
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    timeval tv;
		...
		for(int y_ind = 0; y_ind < DIM_SEL; ++y_ind)
		{
			for(int x_ind = 0; x_ind < DIM_SEL; ++x_ind)
			{
		...
		for(int y_ind = 0; y_ind < DIM_SEL; ++y_ind)
		{
			for(int x_ind = 0; x_ind < DIM_SEL; ++x_ind)
			{
				...
				for(ptrdiff_t wh = 0; wh < DIM_SEL; ++wh)
				{
		...
		int MAX_STEPS = DIM_SEL / DIM;
		int A_sub_row_start = 0; // select minimum row number of A subset
		int AC_row_offset = 0; // row offset from A and C matrices subset
		int BC_col_offset = 0; // column offset of the B matrix subset
		// Write zeros into C
		C_TYPE C_zeros [DIM] = {0,0,0,0,0,0,0,0};
		...
		for(int i = 0; i < MAX_STEPS; i++){
			...
			for(int j = 0; j < MAX_STEPS; j++){
				...
				// Write zeros to C register
				for(ptrdiff_t c_r = 0; c_r < DIM; ++c_r){
					 send_row_C(c_r, C_zeros, afu);
				}

				for(int k = 0; k < MAX_STEPS; k++){
					A_sub_row_start = k * DIM;

					// Write each value of A down.
					fprintf(stdout, "Loading A into AFU...\n");
					for(ptrdiff_t a_r = 0; a_r < DIM; ++a_r)
					{
						send_row_A(a_r, &A_vals[AC_row_offset + a_r][A_sub_row_start], afu);
					}
					...
					{
						send_row_B(b_r, &B_vals[A_sub_row_start + b_r][BC_col_offset], afu);
					}
					...
					for(ptrdiff_t c_r = 0; c_r < DIM; ++c_r)
					{
						unpack_from_C(c_r, &output[AC_row_offset + c_r][BC_col_offset], afu);
					}
									
				}
				
				// Update A row and C row 
				AC_row_offset += DIM;
			}
		...
		for(int r = 0; r < DIM_SEL; ++r)
		{
			for(int c = 0; c < DIM_SEL; ++c)
			{
